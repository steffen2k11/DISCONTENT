...
DISCONTENT.mythicPlusFinderSearchText = DISCONTENT.mythicPlusFinderSearchText or ""
DISCONTENT.mythicPlusFinderRoleFilter = DISCONTENT.mythicPlusFinderRoleFilter or "ALLE"
...
function DISCONTENT:EnsureMythicPlusDB()
    ...
    if type(db.finderEntries) ~= "table" then
        db.finderEntries = {}
    end
    ...
    if type(db.finderRoleFilter) ~= "string" or db.finderRoleFilter == "" then
        db.finderRoleFilter = "ALLE"
    end

    self.mythicPlusArmorFilter = db.guildArmorFilter
    self.mythicPlusFinderRoleFilter = db.finderRoleFilter
...
function DISCONTENT:LeaveMythicPlusTeam(team)
    if not team then
        return false
    end
    ...
    local member, index = self:GetMythicPlusTeamMember(team, playerKey)
    if not member or not index then
        return false
    end
    ...
    if self.mythicPlusSelectedTeamId == team.id then
        self.mythicPlusSelectedEventId = nil
    end

    self:SyncMythicPlusTeam(team)
    return true
end
...
function DISCONTENT:SendMythicPlusAddonMessageToChannel(message, channel, target)
    ...
    local chatType = channel or "WHISPER"
    if chatType == "WHISPER" then
        if not target or target == "" then
            return
        end
        C_ChatInfo.SendAddonMessage(self.mythicPlusPrefix, message, chatType, target)
        return
    end

    C_ChatInfo.SendAddonMessage(self.mythicPlusPrefix, message, chatType)
end

function DISCONTENT:SendMythicPlusAddonMessage(message, target)
    self:SendMythicPlusAddonMessageToChannel(message, "WHISPER", target)
end
...
function DISCONTENT:HandleMythicPlusAddonMessage(prefix, message, channel, sender)
    ...
    elseif msgType == "FGQ" then
        self:HandleMythicPlusFinderRequest(parts, sender)
    elseif msgType == "FG1" then
        self:HandleMythicPlusFinderPacket1(parts, sender)
    elseif msgType == "FG2" then
        self:HandleMythicPlusFinderPacket2(parts, sender)
    elseif msgType == "FG3" then
        self:HandleMythicPlusFinderPacket3(parts, sender)
    elseif msgType == "FGD" then
        self:HandleMythicPlusFinderDelete(parts, sender)
    end
end
...
function DISCONTENT:RefreshMythicPlusPermissionState(team)
    ...
    if self.mythicPlusLeaveButton then
        local canLeave = team and self:IsMemberOfMythicPlusTeam(team) and team.ownerKey ~= self:GetCurrentPlayerKey()
        self.mythicPlusLeaveButton:SetEnabled(canLeave and true or false)
    end
...
function DISCONTENT:RefreshMythicPlusUI()
    ...
    if self.mythicPlusFinderPopup and self.mythicPlusFinderPopup:IsShown() then
        self:RefreshMythicPlusFinderPopup()
    end
end
...
function DISCONTENT:CreateMythicPlusUI()
    ...
    self.mythicPlusSearchGroupButton = createSmallButton(self.mythicPlusTeamPanel, "Suche Gruppe", 110)
    self.mythicPlusSearchGroupButton:SetPoint("TOPLEFT", 12, -12)
    self.mythicPlusSearchGroupButton:SetScript("OnClick", function()
        DISCONTENT:OpenMythicPlusFinderPopup()
    end)

    self.mythicPlusTeamPanel.title = createLabel(self.mythicPlusTeamPanel, "Meine Teams", "GameFontHighlight")
    self.mythicPlusTeamPanel.title:SetPoint("TOPLEFT", 12, -44)

    self.mythicPlusNewButton = createSmallButton(self.mythicPlusTeamPanel, "Neu", 60)
    self.mythicPlusNewButton:SetPoint("TOPLEFT", 12, -68)
    ...
    self.mythicPlusLeaveButton = createSmallButton(self.mythicPlusTeamPanel, "Verlassen", 82)
    self.mythicPlusLeaveButton:SetPoint("LEFT", self.mythicPlusDeleteButton, "RIGHT", 6, 0)
    self.mythicPlusLeaveButton:SetScript("OnClick", function()
        local team = DISCONTENT:GetMythicPlusSelectedTeam()
        if not team then
            return
        end

        if DISCONTENT:LeaveMythicPlusTeam(team) then
            DISCONTENT:SetMythicPlusStatusText("Du hast das Team verlassen.")
            DISCONTENT:RefreshMythicPlusUI()
        else
            DISCONTENT:SetMythicPlusStatusText("Owner können ihr eigenes Team nicht verlassen.")
        end
    end)
...
function DISCONTENT:UpdateMythicPlusLayout()
    ...
    self.mythicPlusTeamListFrame:SetPoint("TOPLEFT", self.mythicPlusTeamPanel, "TOPLEFT", 10, -100)
...
function DISCONTENT:GetMythicPlusFinderEntriesTable()
    local db = self:EnsureMythicPlusDB()
    if db and type(db.finderEntries) ~= "table" then
        db.finderEntries = {}
    end
    return db and db.finderEntries or {}
end

function DISCONTENT:NormalizeMythicPlusFinderRoleFilterValue(value)
    value = tostring(value or "ALLE")
    if value == "ALLE" then
        return value
    end

    for i = 1, #(self.mythicPlusRoleOptions or {}) do
        if value == self.mythicPlusRoleOptions[i] then
            return value
        end
    end
...
function DISCONTENT:GetMythicPlusFinderRoleFilter()
    local db = self:EnsureMythicPlusDB()
    local value = self:NormalizeMythicPlusFinderRoleFilterValue((db and db.finderRoleFilter) or self.mythicPlusFinderRoleFilter)
    if db then
        db.finderRoleFilter = value
    end
    self.mythicPlusFinderRoleFilter = value
    return value
end

function DISCONTENT:SetMythicPlusFinderRoleFilter(value)
    local normalized = self:NormalizeMythicPlusFinderRoleFilterValue(value)
    local db = self:EnsureMythicPlusDB()
    if db then
        db.finderRoleFilter = normalized
    end
    self.mythicPlusFinderRoleFilter = normalized

    if self.mythicPlusFinderPopup and self.mythicPlusFinderPopup.roleFilterButton then
        self.mythicPlusFinderPopup.roleFilterButton:SetText(normalized)
    end
end

function DISCONTENT:CycleMythicPlusFinderRoleFilter()
    local options = { "ALLE" }
    for i = 1, #(self.mythicPlusRoleOptions or {}) do
        options[#options + 1] = self.mythicPlusRoleOptions[i]
    end

    local current = self:GetMythicPlusFinderRoleFilter()
    local nextIndex = 1
    ...
    self:SetMythicPlusFinderRoleFilter(options[nextIndex])
end

function DISCONTENT:CopyMythicPlusFinderEntry(entry)
    if not entry then
        return nil
    end
    ...
    return {
        ...
        classFileName = entry.classFileName or "",
        armorType = entry.armorType or "-",
        role = entry.role or "FLEX",
        playInfo = entry.playInfo or "",
        availability = entry.availability or "",
        goals = entry.goals or "",
        specificNeed = entry.specificNeed or "",
        note = entry.note or "",
        createdAt = tonumber(entry.createdAt) or time(),
        updatedAt = tonumber(entry.updatedAt) or time(),
        updatedBy = entry.updatedBy or "",
    }
end

function DISCONTENT:PruneMythicPlusFinderEntries()
    local entries = self:GetMythicPlusFinderEntriesTable()
    local nowValue = time() or 0
    local cutoff = nowValue - (45 * 24 * 60 * 60)

    for key, entry in pairs(entries) do
        if type(entry) ~= "table" or key == "" then
            entries[key] = nil
        else
...
function DISCONTENT:StoreMythicPlusFinderEntry(entry)
    if not entry or not entry.key or entry.key == "" then
        return
    end

    local entries = self:GetMythicPlusFinderEntriesTable()
    entries[entry.key] = self:CopyMythicPlusFinderEntry(entry)
    self:PruneMythicPlusFinderEntries()
    self:SaveSettings()
end

function DISCONTENT:DeleteMythicPlusFinderEntryLocal(entryKey)
    if not entryKey or entryKey == "" then
        return
    end
...
function DISCONTENT:GetMythicPlusFinderEntry(entryKey)
    if not entryKey or entryKey == "" then
        return nil
    end

    self:PruneMythicPlusFinderEntries()
    return self:GetMythicPlusFinderEntriesTable()[entryKey]
end

function DISCONTENT:ShouldAcceptMythicPlusFinderUpdate(entryKey, updatedAt)
    local current = self:GetMythicPlusFinderEntry(entryKey)
    if not current then
        return true
    end

    return (tonumber(updatedAt) or 0) >= (tonumber(current.updatedAt) or 0)
end

function DISCONTENT:GetMythicPlusFinderDefaultPlayInfo()
    local specName = ""
...
function DISCONTENT:BuildCurrentPlayerMythicPlusFinderEntry(formData)
    local name, realm = self:GetPlayerNameRealm()
    local key = self:GetCharacterKey(name, realm)
    local guildMember = self:GetGuildMemberByKey(key)
    local localizedClass, classFileName = UnitClass("player")
    local current = self:GetMythicPlusFinderEntry(key)

    local roleValue = self:MythicPlusSanitize(formData.role or "FLEX", 8)
    if roleValue == "" then
        roleValue = "FLEX"
    end
    ...
    return {
        ...
        name = self:SafeName(name),
        realm = realm,
        className = guildMember and guildMember.className or localizedClass or "",
        classFileName = guildMember and guildMember.classFileName or classFileName or "",
        armorType = guildMember and self:GetArmorTypeForClass(guildMember.classFileName) or self:GetArmorTypeForClass(classFileName),
        role = roleValue,
        playInfo = self:MythicPlusSanitize(formData.playInfo, 60),
        availability = self:MythicPlusSanitize(formData.availability, 100),
        goals = self:MythicPlusSanitize(formData.goals, 100),
        specificNeed = self:MythicPlusSanitize(formData.specificNeed, 100),
        note = self:MythicPlusSanitize(formData.note, 100),
        createdAt = current and (tonumber(current.createdAt) or time()) or time(),
        updatedAt = time(),
        updatedBy = self:GetCurrentPlayerDisplayName(),
    }
end

function DISCONTENT:GetOwnedMythicPlusTeams()
    local result = {}
    local playerKey = self:GetCurrentPlayerKey()
    local teams = self:GetMythicPlusVisibleTeams()
    ...
    for i = 1, #teams do
        local team = teams[i]
        if team and team.ownerKey == playerKey then
...
function DISCONTENT:CanAddFinderEntryToOwnedTeams(entry)
    if not entry or not entry.key or entry.key == "" then
        return false
    end

    local teams = self:GetOwnedMythicPlusTeams()
    for i = 1, #teams do
        if not self:GetMythicPlusTeamMember(teams[i], entry.key) then
            return true
        end
    end
...
function DISCONTENT:GetFilteredMythicPlusFinderEntries()
    self:PruneMythicPlusFinderEntries()

    local list = {}
    local searchText = self:NormalizeText(self.mythicPlusFinderSearchText or "")
    local roleFilter = self:GetMythicPlusFinderRoleFilter()

    for key, entry in pairs(self:GetMythicPlusFinderEntriesTable()) do
        if type(entry) == "table" and key ~= "" then
            local matchesRole = (roleFilter == "ALLE" or (entry.role or "") == roleFilter)
            local haystack = table.concat({
                ...
                entry.goals or "",
                entry.specificNeed or "",
                entry.note or "",
            }, " ")
            local matchesSearch = (searchText == "" or string.find(self:NormalizeText(haystack), searchText, 1, true))

            if matchesRole and matchesSearch then
                list[#list + 1] = entry
            end
        end
    end
    ...
    table.sort(list, function(a, b)
        ...
        return self:NormalizeText((a.name or "") .. "-" .. (a.realm or "")) < self:NormalizeText((b.name or "") .. "-" .. (b.realm or ""))
    end)

    return list
end

function DISCONTENT:BuildMythicPlusFinderPacket1(entry)
    return table.concat({
        "FG1",
        self:MythicPlusSanitize(entry.key, 48),
        tostring(tonumber(entry.updatedAt) or time()),
        self:MythicPlusSanitize(entry.name, 24),
        self:MythicPlusSanitize(entry.realm, 24),
        self:MythicPlusSanitize(entry.classFileName, 16),
        self:MythicPlusSanitize(entry.className, 18),
        self:MythicPlusSanitize(entry.armorType, 8),
        self:MythicPlusSanitize(entry.role, 8),
        self:MythicPlusSanitize(entry.playInfo, 36),
    }, "^")
end

function DISCONTENT:BuildMythicPlusFinderPacket2(entry)
    return table.concat({
        "FG2",
        self:MythicPlusSanitize(entry.key, 48),
        tostring(tonumber(entry.updatedAt) or time()),
        self:MythicPlusSanitize(entry.availability, 80),
        self:MythicPlusSanitize(entry.goals, 80),
    }, "^")
end

function DISCONTENT:BuildMythicPlusFinderPacket3(entry)
    return table.concat({
        "FG3",
        self:MythicPlusSanitize(entry.key, 48),
        tostring(tonumber(entry.updatedAt) or time()),
        self:MythicPlusSanitize(entry.specificNeed, 80),
        self:MythicPlusSanitize(entry.note, 80),
    }, "^")
end

function DISCONTENT:BuildMythicPlusFinderDeletePacket(entryKey, updatedAt)
    return table.concat({
        "FGD",
        self:MythicPlusSanitize(entryKey, 48),
        tostring(tonumber(updatedAt) or time()),
    }, "^")
end

function DISCONTENT:BuildMythicPlusFinderRequestPacket()
    return table.concat({
        "FGQ",
        self:MythicPlusSanitize(self:GetCurrentPlayerKey(), 48),
    }, "^")
end

function DISCONTENT:BroadcastMythicPlusFinderEntry(entry, target)
    if not entry then
        return
    end
    ...
    local messages = {
        self:BuildMythicPlusFinderPacket1(entry),
        self:BuildMythicPlusFinderPacket2(entry),
        self:BuildMythicPlusFinderPacket3(entry),
    }
    ...
    for i = 1, #messages do
        ...
        C_Timer.After(delay, function()
            if DISCONTENT then
                DISCONTENT:SendMythicPlusAddonMessageToChannel(payload, channel, target)
            end
        end)
    end
end

function DISCONTENT:BroadcastMythicPlusFinderDelete(entryKey)
    if not entryKey or entryKey == "" or not IsInGuild() then
        return
    end

    self:SendMythicPlusAddonMessageToChannel(self:BuildMythicPlusFinderDeletePacket(entryKey, time()), "GUILD")
end

function DISCONTENT:RequestMythicPlusFinderEntries()
    if not IsInGuild() then
        return
    end

    self:SendMythicPlusAddonMessageToChannel(self:BuildMythicPlusFinderRequestPacket(), "GUILD")
end

function DISCONTENT:HandleMythicPlusFinderRequest(parts, sender)
    local requesterKey = parts[2] or ""
    local senderName, senderRealm = normalizeNameRealm(sender or "")
    local senderKey = self:GetCharacterKey(senderName, senderRealm)
    local myKey = self:GetCurrentPlayerKey()
    if requesterKey == "" or senderKey == myKey then
        return
    end

    local entry = self:GetMythicPlusFinderEntry(myKey)
    if entry then
        self:BroadcastMythicPlusFinderEntry(entry, sender)
    end
end

function DISCONTENT:GetOrCreateIncomingMythicPlusFinderEntry(entryKey, updatedAt)
    if not entryKey or entryKey == "" or not self:ShouldAcceptMythicPlusFinderUpdate(entryKey, updatedAt) then
        return nil
    end

    local current = self:GetMythicPlusFinderEntry(entryKey)
    if current and (tonumber(current.updatedAt) or 0) == (tonumber(updatedAt) or 0) then
        return self:CopyMythicPlusFinderEntry(current)
    end

    local name, realm = normalizeNameRealm(entryKey)
    return {
        key = entryKey,
        name = self:SafeName(name),
        realm = realm,
...
function DISCONTENT:HandleMythicPlusFinderPacket1(parts, sender)
    local entryKey = parts[2] or ""
    local updatedAt = tonumber(parts[3]) or 0
    local entry = self:GetOrCreateIncomingMythicPlusFinderEntry(entryKey, updatedAt)
    if not entry then
        return
    end

    entry.name = parts[4] or entry.name or ""
    entry.realm = parts[5] or entry.realm or ""
    entry.classFileName = parts[6] or ""
    entry.className = parts[7] or ""
    entry.armorType = parts[8] or "-"
    entry.role = parts[9] or "FLEX"
    entry.playInfo = parts[10] or ""
    entry.updatedBy = sender or entry.updatedBy or ""
    self:StoreMythicPlusFinderEntry(entry)
...
function DISCONTENT:HandleMythicPlusFinderPacket2(parts, sender)
    local entryKey = parts[2] or ""
    local updatedAt = tonumber(parts[3]) or 0
    local entry = self:GetOrCreateIncomingMythicPlusFinderEntry(entryKey, updatedAt)
    if not entry then
    ...
    entry.availability = parts[4] or ""
    entry.goals = parts[5] or ""
    entry.updatedBy = sender or entry.updatedBy or ""
    self:StoreMythicPlusFinderEntry(entry)
...
function DISCONTENT:HandleMythicPlusFinderPacket3(parts, sender)
    local entryKey = parts[2] or ""
    local updatedAt = tonumber(parts[3]) or 0
    local entry = self:GetOrCreateIncomingMythicPlusFinderEntry(entryKey, updatedAt)
    if not entry then
    ...
    entry.specificNeed = parts[4] or ""
    entry.note = parts[5] or ""
    entry.updatedBy = sender or entry.updatedBy or ""
    self:StoreMythicPlusFinderEntry(entry)
...
function DISCONTENT:HandleMythicPlusFinderDelete(parts, sender)
    local entryKey = parts[2] or ""
    local updatedAt = tonumber(parts[3]) or 0
    local current = self:GetMythicPlusFinderEntry(entryKey)
    if not current then
    ...
    if (tonumber(current.updatedAt) or 0) > updatedAt then
        return
    end

    self:DeleteMythicPlusFinderEntryLocal(entryKey)
...
function DISCONTENT:OpenMythicPlusFinderFormPopup()
    if not self.mythicPlusFinderFormPopup then
        local popup = CreateFrame("Frame", "DISCONTENTMythicPlusFinderFormPopup", UIParent, "BackdropTemplate")
        ...
        popup:SetScript("OnDragStop", function(frame)
            ...
            DISCONTENT:SaveFramePosition(frame, "mythicPlusFinderFormPosition")
        end)
        popup:SetBackdrop({
            bgFile = "Interface/Tooltips/UI-Tooltip-Background",
            edgeFile = "Interface/Tooltips/UI-Tooltip-Border",
        ...
        popup.title:SetText("M+ Gruppe suchen")

        popup.closeButton = CreateFrame("Button", nil, popup, "UIPanelCloseButton")
        popup.closeButton:SetPoint("TOPRIGHT", -4, -4)

        popup.charLabel = popup:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
        popup.charLabel:SetPoint("TOPLEFT", 16, -44)
        popup.charLabel:SetText("Charakter")

        popup.charValue = popup:CreateFontString(nil, "OVERLAY", "GameFontNormal")
        popup.charValue:SetPoint("TOPLEFT", 16, -62)
        popup.charValue:SetText("-")

        popup.roleLabel = popup:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
        popup.roleLabel:SetPoint("TOPLEFT", 16, -88)
        popup.roleLabel:SetText("Rolle")

        popup.roleButton = createSmallButton(popup, "FLEX", 86)
        popup.roleButton:SetPoint("LEFT", popup.roleLabel, "RIGHT", 10, 0)
        popup.roleButton:SetScript("OnClick", function(button)
            local currentRole = button:GetText() or "FLEX"
            local nextRole = "FLEX"
            for i = 1, #(DISCONTENT.mythicPlusRoleOptions or {}) do
                if DISCONTENT.mythicPlusRoleOptions[i] == currentRole then
                    nextRole = DISCONTENT.mythicPlusRoleOptions[(i % #DISCONTENT.mythicPlusRoleOptions) + 1]
                    break
                end
            end
            button:SetText(nextRole)
        end)

        local function createInput(labelText, anchor, offsetY)
        ...
        popup.playInfoLabel, popup.playInfoInput = createInput("Was spielst du? (z.B. Main / Spec / Erfahrung)", popup.roleLabel, -118)
        popup.availabilityLabel, popup.availabilityInput = createInput("Wann hast du Zeit?", popup.playInfoLabel, -170)
        popup.goalsLabel, popup.goalsInput = createInput("Welche Ziele hast du?", popup.availabilityLabel, -222)
        popup.specificLabel, popup.specificInput = createInput("Konkreter Termin / Übergang / Zweck", popup.goalsLabel, -274)
        popup.noteLabel, popup.noteInput = createInput("Zusatz", popup.specificLabel, -326)

        popup.saveButton = createSmallButton(popup, "Suche speichern", 120)
        popup.saveButton:SetPoint("BOTTOMRIGHT", -16, 16)
        popup.saveButton:SetScript("OnClick", function()
            local entry = DISCONTENT:BuildCurrentPlayerMythicPlusFinderEntry({
                role = popup.roleButton:GetText(),
                playInfo = popup.playInfoInput:GetText() or "",
                availability = popup.availabilityInput:GetText() or "",
                goals = popup.goalsInput:GetText() or "",
                specificNeed = popup.specificInput:GetText() or "",
                note = popup.noteInput:GetText() or "",
            })

            DISCONTENT:StoreMythicPlusFinderEntry(entry)
            DISCONTENT:BroadcastMythicPlusFinderEntry(entry)
            DISCONTENT:SetMythicPlusStatusText("Deine Gruppensuche wurde gespeichert.")
            popup:Hide()
            if DISCONTENT.mythicPlusFinderPopup and DISCONTENT.mythicPlusFinderPopup:IsShown() then
                DISCONTENT:RefreshMythicPlusFinderPopup()
            end
        end)

        popup.deleteButton = createSmallButton(popup, "Suche beenden", 120)
        popup.deleteButton:SetPoint("RIGHT", popup.saveButton, "LEFT", -8, 0)
        popup.deleteButton:SetScript("OnClick", function()
            local entryKey = DISCONTENT:GetCurrentPlayerKey()
            DISCONTENT:DeleteMythicPlusFinderEntryLocal(entryKey)
            DISCONTENT:BroadcastMythicPlusFinderDelete(entryKey)
            DISCONTENT:SetMythicPlusStatusText("Deine Gruppensuche wurde entfernt.")
            popup:Hide()
            if DISCONTENT.mythicPlusFinderPopup and DISCONTENT.mythicPlusFinderPopup:IsShown() then
                DISCONTENT:RefreshMythicPlusFinderPopup()
            end
        ...
        popup.cancelButton = createSmallButton(popup, "Abbrechen", 100)
        popup.cancelButton:SetPoint("RIGHT", popup.deleteButton, "LEFT", -8, 0)
        popup.cancelButton:SetScript("OnClick", function()
            popup:Hide()
        end)

        self.mythicPlusFinderFormPopup = popup
    end

    local popup = self.mythicPlusFinderFormPopup
    local entry = self:GetMythicPlusFinderEntry(self:GetCurrentPlayerKey())
    popup.charValue:SetText(self:GetCurrentPlayerDisplayName())
    popup.roleButton:SetText((entry and entry.role) or "FLEX")
    local playInfoText = self:GetMythicPlusFinderDefaultPlayInfo()
    if entry and type(entry.playInfo) == "string" and entry.playInfo ~= "" then
        playInfoText = entry.playInfo
    end
    popup.playInfoInput:SetText(playInfoText)
    popup.availabilityInput:SetText((entry and entry.availability) or "")
    popup.goalsInput:SetText((entry and entry.goals) or "")
    popup.specificInput:SetText((entry and entry.specificNeed) or "")
    popup.noteInput:SetText((entry and entry.note) or "")
    popup.deleteButton:SetEnabled(entry and true or false)
    popup:Show()
    self:RestoreFramePosition(popup, "mythicPlusFinderFormPosition", "CENTER", "CENTER", 110, 40)
    popup.playInfoInput:SetFocus()
    popup.playInfoInput:HighlightText()
end

function DISCONTENT:AddFinderEntryToOwnedTeam(entry, teamId)
    if not entry or not teamId then
        return false
    end

    local team = self:GetMythicPlusTeam(teamId)
    if not team or team.ownerKey ~= self:GetCurrentPlayerKey() then
        return false
    end

    if self:GetMythicPlusTeamMember(team, entry.key) then
        self:SetMythicPlusStatusText("Spieler ist bereits in diesem Team.")
        return false
    end
    ...
    if guildMember then
        success = self:AddGuildMemberToMythicPlusTeam(team, guildMember)
    else
        success = self:AddExternalMemberToMythicPlusTeam(team, entry.name, entry.realm)
    end

    if success then
        self:SyncMythicPlusTeam(team)
        self:SetMythicPlusStatusText(string.format("%s wurde zu '%s' hinzugefügt.", self:GetMythicPlusMemberDisplayName(entry), team.name or "-"))
        return true
    end

    self:SetMythicPlusStatusText("Spieler konnte nicht hinzugefügt werden.")
    return false
end

function DISCONTENT:OpenMythicPlusFinderTeamPicker(entry)
    if not entry then
        return
    end

    if not self.mythicPlusFinderTeamPicker then
        local popup = CreateFrame("Frame", "DISCONTENTMythicPlusFinderTeamPicker", UIParent, "BackdropTemplate")
        ...
        popup:SetScript("OnDragStop", function(frame)
            frame:StopMovingOrSizing()
            DISCONTENT:SaveFramePosition(frame, "mythicPlusFinderTeamPickerPosition")
        end)
        popup:SetBackdrop({
            bgFile = "Interface/Tooltips/UI-Tooltip-Background",
            edgeFile = "Interface/Tooltips/UI-Tooltip-Border",
        ...
        popup.title = popup:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge")
        popup.title:SetPoint("TOPLEFT", 16, -14)
        popup.title:SetText("Zu M+ Team hinzufügen")

        popup.closeButton = CreateFrame("Button", nil, popup, "UIPanelCloseButton")
        popup.closeButton:SetPoint("TOPRIGHT", -4, -4)

        popup.targetText = popup:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
        popup.targetText:SetPoint("TOPLEFT", 16, -46)
        popup.targetText:SetWidth(370)
        popup.targetText:SetJustifyH("LEFT")
        popup.targetText:SetText("-")

        popup.emptyText = popup:CreateFontString(nil, "OVERLAY", "GameFontNormal")
        popup.emptyText:SetPoint("TOPLEFT", 16, -76)
        popup.emptyText:SetWidth(360)
        popup.emptyText:SetJustifyH("LEFT")
        popup.emptyText:SetText("Du besitzt aktuell keine eigenen M+ Teams im Addon.")

        popup.rows = {}
        for i = 1, 6 do
            local row = CreateFrame("Frame", nil, popup, "BackdropTemplate")
            row:SetHeight(34)
            row:SetBackdrop({
                bgFile = "Interface/Tooltips/UI-Tooltip-Background",
                edgeFile = "Interface/Tooltips/UI-Tooltip-Border",
            ...
            row:SetBackdropColor(0.08, 0.08, 0.08, 0.92)
            row:SetBackdropBorderColor(0.42, 0.42, 0.42, 0.95)
            row:SetPoint("TOPLEFT", 16, -74 - ((i - 1) * 40))
            row:SetPoint("TOPRIGHT", -16, -74 - ((i - 1) * 40))
            ...
            row.addButton = createSmallButton(row, "Hinzufügen", 90)
            row.addButton:SetPoint("RIGHT", -10, 0)
            row.addButton:SetScript("OnClick", function(button)
                if popup.entry and button.teamId and DISCONTENT:AddFinderEntryToOwnedTeam(popup.entry, button.teamId) then
                    popup:Hide()
                    if DISCONTENT.mythicPlusFinderPopup and DISCONTENT.mythicPlusFinderPopup:IsShown() then
                        DISCONTENT:RefreshMythicPlusFinderPopup()
                    end
        ...
        self.mythicPlusFinderTeamPicker = popup
    end

    local popup = self.mythicPlusFinderTeamPicker
    popup.entry = entry
    popup.targetText:SetText("Spieler: " .. self:GetMythicPlusMemberDisplayName(entry))

    local teams = self:GetOwnedMythicPlusTeams()
    popup.emptyText:SetShown(#teams == 0)

    for i = 1, #(popup.rows or {}) do
        local row = popup.rows[i]
        local team = teams[i]
        if team then
            local alreadyInTeam = self:GetMythicPlusTeamMember(team, entry.key) and true or false
            row.nameText:SetText((team.name or "-") .. " | Mitglieder: " .. tostring(self:GetMythicPlusTeamCount(team)))
            row.addButton.teamId = team.id
            row.addButton:SetEnabled(not alreadyInTeam)
            row.addButton:SetText(alreadyInTeam and "Bereits drin" or "Hinzufügen")
            row:Show()
        else
            row:Hide()
        end
    ...
    popup:Show()
    self:RestoreFramePosition(popup, "mythicPlusFinderTeamPickerPosition", "CENTER", "CENTER", 150, 70)
end

function DISCONTENT:RefreshMythicPlusFinderRows()
    if not self.mythicPlusFinderPopup or not self.mythicPlusFinderPopup.listContent then
        return
    end

    local popup = self.mythicPlusFinderPopup
    local entries = self:GetFilteredMythicPlusFinderEntries()
    popup.rows = popup.rows or {}
    local contentWidth = math.max(640, (popup.listFrame:GetWidth() or 680) - 24)

    for i = 1, #entries do
        local row = popup.rows[i]
        if not row then
            row = CreateFrame("Frame", nil, popup.listContent, "BackdropTemplate")
            row:SetHeight(112)
            row:SetBackdrop({
                bgFile = "Interface/Tooltips/UI-Tooltip-Background",
                edgeFile = "Interface/Tooltips/UI-Tooltip-Border",
            ...
            row.titleText:SetPoint("TOPLEFT", 10, -10)
            row.titleText:SetJustifyH("LEFT")
            row.titleText:SetJustifyV("TOP")

            row.info1 = row:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
            row.info1:SetJustifyH("LEFT")
            row.info1:SetJustifyV("TOP")
            row.info1:SetPoint("TOPLEFT", row.titleText, "BOTTOMLEFT", 0, -8)

            row.info2 = row:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
            row.info2:SetJustifyH("LEFT")
            row.info2:SetJustifyV("TOP")
            row.info2:SetPoint("TOPLEFT", row.info1, "BOTTOMLEFT", 0, -6)

            row.info3 = row:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
            row.info3:SetJustifyH("LEFT")
            row.info3:SetJustifyV("TOP")
            row.info3:SetPoint("TOPLEFT", row.info2, "BOTTOMLEFT", 0, -6)

            row.updatedText = row:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
            row.updatedText:SetJustifyH("LEFT")
            row.updatedText:SetPoint("BOTTOMLEFT", 10, 10)

            row.whisperButton = createSmallButton(row, "Whisper", 70)
            row.whisperButton:SetPoint("TOPRIGHT", -10, -10)
            row.whisperButton:SetScript("OnClick", function(button)
                ...
                DISCONTENT:OpenMythicPlusTextPrompt(
                    "Spieler anschreiben",
                    "Whisper an " .. DISCONTENT:GetMythicPlusMemberDisplayName(button.entry),
                    "",
                    function(text)
                        DISCONTENT:SendWhisperToMythicPlusMember(button.entry, text)
                    end
                )
            end)

            row.addButton = createSmallButton(row, "Zu Team", 70)
            row.addButton:SetPoint("TOPRIGHT", row.whisperButton, "BOTTOMRIGHT", 0, -6)
            row.addButton:SetScript("OnClick", function(button)
                if button.entry then
                    DISCONTENT:OpenMythicPlusFinderTeamPicker(button.entry)
                end
            end)
        ...
        row:SetWidth(contentWidth)
        row:ClearAllPoints()
        row:SetPoint("TOPLEFT", popup.listContent, "TOPLEFT", 2, -((i - 1) * 118))
        row:SetPoint("TOPRIGHT", popup.listContent, "TOPRIGHT", -2, -((i - 1) * 118))

        row.titleText:SetWidth(contentWidth - 190)
        row.info1:SetWidth(contentWidth - 190)
        row.info2:SetWidth(contentWidth - 190)
        row.info3:SetWidth(contentWidth - 190)

        row.titleText:SetText((self:GetMythicPlusMemberDisplayName(entry) or "-") .. " | " .. (entry.role or "FLEX") .. " | " .. (entry.playInfo ~= "" and entry.playInfo or "Keine Angabe"))
        row.titleText:SetTextColor(r, g, b, 1)
        row.info1:SetText("Zeiten: " .. ((entry.availability ~= "" and entry.availability) or "-") )
        row.info2:SetText("Ziele: " .. ((entry.goals ~= "" and entry.goals) or "-") )

        local extraBits = {}
        if entry.specificNeed and entry.specificNeed ~= "" then
            extraBits[#extraBits + 1] = "Gesucht: " .. entry.specificNeed
        end
        if entry.note and entry.note ~= "" then
            extraBits[#extraBits + 1] = "Zusatz: " .. entry.note
        end
        row.info3:SetText(#extraBits > 0 and table.concat(extraBits, " | ") or "Gesucht: -")
        row.updatedText:SetText("Aktualisiert: " .. self:FormatUpdatedAt(entry.updatedAt))

        row.whisperButton.entry = entry
        row.addButton.entry = entry
        row.addButton:SetEnabled(self:CanAddFinderEntryToOwnedTeams(entry))
        row:Show()
    end

    for i = #entries + 1, #(popup.rows or {}) do
        popup.rows[i]:Hide()
    end

    popup.listContent:SetWidth(contentWidth)
    popup.listContent:SetHeight(math.max(280, #entries * 118 + 8))

    if popup.emptyState then
        popup.emptyState:SetShown(#entries == 0)
    end
end

function DISCONTENT:RefreshMythicPlusFinderPopup()
    if not self.mythicPlusFinderPopup then
        return
    end

    local popup = self.mythicPlusFinderPopup
    if popup.roleFilterButton then
        popup.roleFilterButton:SetText(self:GetMythicPlusFinderRoleFilter())
    end

    local ownEntry = self:GetMythicPlusFinderEntry(self:GetCurrentPlayerKey())
    if popup.mySearchButton then
        popup.mySearchButton:SetText(ownEntry and "Suche bearbeiten" or "Gruppe suchen")
    end

    self:RefreshMythicPlusFinderRows()
end

function DISCONTENT:OpenMythicPlusFinderPopup()
    if not self.mythicPlusFinderPopup then
        local popup = CreateFrame("Frame", "DISCONTENTMythicPlusFinderPopup", UIParent, "BackdropTemplate")
        popup:SetSize(780, 560)
        popup:SetFrameStrata("FULLSCREEN_DIALOG")
        popup:SetToplevel(true)
        popup:SetClampedToScreen(true)
        popup:SetMovable(true)
        ...
        popup:SetScript("OnDragStop", function(frame)
            frame:StopMovingOrSizing()
            DISCONTENT:SaveFramePosition(frame, "mythicPlusFinderPopupPosition")
        end)
        popup:SetBackdrop({
            bgFile = "Interface/Tooltips/UI-Tooltip-Background",
            edgeFile = "Interface/Tooltips/UI-Tooltip-Border",
        ...
        popup.title = popup:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge")
        popup.title:SetPoint("TOPLEFT", 16, -14)
        popup.title:SetText("Suche Gruppe")

        popup.closeButton = CreateFrame("Button", nil, popup, "UIPanelCloseButton")
        popup.closeButton:SetPoint("TOPRIGHT", -4, -4)

        popup.searchLabel = popup:CreateFontString(nil, "OVERLAY", "GameFontNormal")
        popup.searchLabel:SetPoint("TOPLEFT", 16, -46)
        popup.searchLabel:SetText("Filter")

        popup.searchBox = CreateFrame("EditBox", nil, popup, "InputBoxTemplate")
        popup.searchBox:SetAutoFocus(false)
        popup.searchBox:SetSize(220, 24)
        popup.searchBox:SetPoint("LEFT", popup.searchLabel, "RIGHT", 8, 0)
        popup.searchBox:SetScript("OnTextChanged", function(editBox)
            DISCONTENT.mythicPlusFinderSearchText = editBox:GetText() or ""
            DISCONTENT:RefreshMythicPlusFinderPopup()
        end)

        popup.roleFilterLabel = popup:CreateFontString(nil, "OVERLAY", "GameFontNormal")
        popup.roleFilterLabel:SetPoint("LEFT", popup.searchBox, "RIGHT", 18, 0)
        popup.roleFilterLabel:SetText("Rolle")

        popup.roleFilterButton = createSmallButton(popup, "ALLE", 82)
        popup.roleFilterButton:SetPoint("LEFT", popup.roleFilterLabel, "RIGHT", 8, 0)
        popup.roleFilterButton:RegisterForClicks("LeftButtonUp", "RightButtonUp")
        popup.roleFilterButton:SetScript("OnClick", function(_, mouseButton)
            if mouseButton == "RightButton" then
                DISCONTENT:SetMythicPlusFinderRoleFilter("ALLE")
            else
                DISCONTENT:CycleMythicPlusFinderRoleFilter()
            end
            DISCONTENT:RefreshMythicPlusFinderPopup()
        end)

        popup.refreshButton = createSmallButton(popup, "Aktualisieren", 94)
        popup.refreshButton:SetPoint("TOPRIGHT", -146, -42)
        popup.refreshButton:SetScript("OnClick", function()
            DISCONTENT:RequestMythicPlusFinderEntries()
            DISCONTENT:SetMythicPlusStatusText("Gruppensuche wird aktualisiert.")
        end)

        popup.mySearchButton = createSmallButton(popup, "Gruppe suchen", 118)
        popup.mySearchButton:SetPoint("LEFT", popup.refreshButton, "RIGHT", 8, 0)
        popup.mySearchButton:SetScript("OnClick", function()
            DISCONTENT:OpenMythicPlusFinderFormPopup()
        end)

        popup.listFrame = CreateFrame("ScrollFrame", "DISCONTENTMythicPlusFinderScrollFrame", popup, "UIPanelScrollFrameTemplate")
        popup.listFrame:SetPoint("TOPLEFT", 16, -78)
        popup.listFrame:SetPoint("BOTTOMRIGHT", -30, 16)

        popup.listContent = CreateFrame("Frame", nil, popup.listFrame)
        popup.listContent:SetSize(700, 400)
        popup.listFrame:SetScrollChild(popup.listContent)

        popup.emptyState = popup.listContent:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge")
        popup.emptyState:SetPoint("TOP", 0, -120)
        popup.emptyState:SetText("Aktuell sucht niemand eine Gruppe.")
        popup.emptyState:SetTextColor(0.8, 0.8, 0.8, 1)

        self.mythicPlusFinderPopup = popup
    end

    local popup = self.mythicPlusFinderPopup
    popup.searchBox:SetText(self.mythicPlusFinderSearchText or "")
    popup:Show()
    self:RestoreFramePosition(popup, "mythicPlusFinderPopupPosition", "CENTER", "CENTER", 80, 30)
    self:RequestMythicPlusFinderEntries()
    self:RefreshMythicPlusFinderPopup()
end
