...
function DISCONTENT:CreateOrUpdateMythicPlusEvent(team, eventId, title, dateText, timeText, note)
    if not team then
        return nil
    end
...
function DISCONTENT:RefreshMythicPlusScheduleRows(team)
    ...
    for i = 1, #events do
        ...
        row:SetWidth(contentWidth)
        row:ClearAllPoints()
        row:SetPoint("TOPLEFT", self.mythicPlusScheduleListContent, "TOPLEFT", 2, -((i - 1) * 90))
        row:SetPoint("TOPRIGHT", self.mythicPlusScheduleListContent, "TOPRIGHT", -2, -((i - 1) * 90))
        ...
        row.statusText:SetText("Dein Status: " .. myStatus)
    ...
    self.mythicPlusScheduleListContent:SetHeight(math.max(220, #events * 90 + 8))
end
